 ...
 * Name(s): Caleb Kim
 *
 * What I thought about this assignment: Good assignment overall. I learned a lot, but it wasn't too difficult.
 *
 *
 */
...
public final class Heap<T> {
    ...
    void bubbleUp(int h) {
        ...
        if (h >= size) return;
        int parent= (h - 1) / 2;
        while (h > 0 && compareTo(h, parent) == 1) {
            swap(h, parent);
            h= parent;
            parent= (parent - 1) / 2;
        }
    ...
    void bubbleDown(int h) {
        ...
        int bigChild;
        while (c1 < size) {
            if (c2 >= size || compareTo(c1, c2) >= 0) {
                bigChild= c1;
            } else {
                bigChild= c2;
            }
            if (compareTo(bigChild, h) <= 0) return;
            swap(bigChild, h);
            h= bigChild;
            c1= 2 * bigChild + 1;
            c2= 2 * bigChild + 2;
